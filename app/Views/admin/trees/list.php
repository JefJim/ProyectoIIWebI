...
<body class="h-full">
    <div class="min-h-full">
        <div class="flex justify-between items-center bg-green-900 p-4 text-white">
            ...
            <a href="<?= base_url('admin/arboles/crear') ?>" class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded text-sm">Agregar Árbol</a>
        </div>
                ...
                <tbody>
                    <?php foreach ($trees as $tree): ?>
                        <tr class="bg-white border-b hover:bg-green-50">
                            ...
                            <td class="px-6 py-4">
                                <?php if ($tree['foto']): ?>
                                    <img src="<?= base_url('uploads/trees/' . esc($tree['foto'])) ?>" alt="Foto" class="h-16 w-16 object-cover rounded">
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4 space-x-2">
                                <a href="<?= base_url('admin/arboles/editar/' . esc($tree['id'])) ?>" class="text-blue-600 hover:text-blue-800">Editar</a>
                                <a href="<?= base_url('admin/arboles/eliminar/' . esc($tree['id'])) ?>" onclick="return confirm('¿Estás seguro de eliminar este árbol?')" class="text-red-600 hover:text-red-800">Eliminar</a>
                                <a href="/admin/arboles/<?= esc($tree['id']) ?>/historial" class="text-green-600 hover:text-green-800">Ver Historial</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
